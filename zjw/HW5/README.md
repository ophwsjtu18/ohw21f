### YOLO算法识别手机，根据手机位置控制我的世界人物前后左右跑动。代码和截屏上传github
```python
import numpy as np
import cv2
import os
...
yolo_dir = 'E:\\yolov3'  # YOLO文件路径一定不能有中文在路径里面
...
while(True):
    ...
    start = time.time()
    (H, W) = frame.shape[:2]
    blobImg = cv2.dnn.blobFromImage(frame, 1.0/255.0, (416, 416), None, True, False)  ## net需要的输入是blob格式的，用blobFromImage这个函数来转格式
    ...
    if len(idxs) > 0:
        for i in idxs.flatten(): # indxs是二维的，第0维是输出层，所以这里把它展平成1维
            ...
            if(labels[classIDs[i]] == "cell phone"):
                ...
                print(x,' ', y, ' ', w, ' ',h)
                if(x < 180):
                    mc.player.setTilePos(pos.x+2, pos.y, pos.z)
                elif(x > 280):
                    mc.player.setTilePos(pos.x-2, pos.y, pos.z)
                
    end = time.time()
    print("YOLO1 took {:.6f} seconds".format(end - start))
    cv2.imshow('target detect result1', frame)
    cv2.waitKey(500)
"""
...
![HW5.img](https://github.com/ophwsjtu18/ohw21f/blob/main/zjw/zjw.assets/HW5.png)
